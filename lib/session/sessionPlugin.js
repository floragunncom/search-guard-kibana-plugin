...
internals.config = Joi.object({
  ...
  sessionStorageFactory: Joi.required(),
}).required();
  ...
  server.ext('onPreAuth', function (request, h) {
    request.auth.sgSessionStorage = {
      ...
      _handleAuthResponse: function (credentials, authResponse, additionalAuthHeaders = {}) {
        ...
        settings.sessionStorageFactory.asScoped(request).set(authResponse.session);
      ...
      setAuthInfo: function (
        ...
        if (config.get('searchguard.auth.disable_authinfo_cache') === true) {
          return;
        }
      ...
      getAuthInfo: async function () {
        ...
        if (
          ...
          config.get('searchguard.auth.disable_authinfo_cache') !== true
        ) {
