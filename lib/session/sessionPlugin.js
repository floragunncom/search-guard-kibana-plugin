...
internals.config = Joi.object({
    ...
    extraCookieName: Joi.string(),
    extraCookies: Joi.number().integer(),
}).required();
        ...
        request.auth.sgSessionStorage = {
            ...
            authenticateWithHeaders: async function (headers, credentials = {}, options = {}) {
                try {
                    ...
                    let sessionTTL = server.config().get('searchguard.session.ttl')
            ...
            _handleAuthResponse: function (credentials, authResponse, additionalAuthHeaders = {}) {
                ...
                try {
                  this.putExtraStorage(authResponse.session.credentials.authHeaderValue);
                } catch (error) {
                  console.log(error);
                }

                if (settings.authType === 'openid') {
                  // This is a dummy value just to have some credentials.
                  // The real cookie value will be extracted from the extra cookies.
                  authResponse.session.credentials.authHeaderValue = 'placeholder';
                }
            ...
            clear: function () {
                ...
                for (let i = 0; i < settings.extraCookies; i++) {
                  h.unstate(settings.extraCookieName + '_' + i);
                }
            },
            ...
            getStorage: function (key, whenMissing = null) {
                let storage = request.state[settings.storageCookieName];
            ...
            /**
             * Get a value from the extra cookie(s)
             * @param key
             * @param value
             */
            getExtraStorage: function () {
              let value = '';
              for (let i = 0; i < settings.extraCookies; i++) {
                value = value + request.state[settings.extraCookieName + '_' + i] || '';
              }
            ...
            /**
             * Store a value in the extra cookie(s)
             * @param value
             */
            putExtraStorage: function (value) {
              const partLength = Math.ceil(value.length / settings.extraCookies);
              for (let i = 0; i < settings.extraCookies; i++) {
                h.state(settings.extraCookieName + '_' + i, value.slice(i * partLength, ( i * partLength) + partLength));
              }
            },
            ...
            setAuthInfo: function (user_name, backend_roles, sg_roles, sg_tenants, user_requested_tenant) {
                if (server.config().get('searchguard.auth.disable_authinfo_cache') === true) {
                  return;
                }
            ...
            getAuthInfo: async function () {
                try {
                    ...
                    request.log(['error', 'searchguard'], `Could not get auth info ${error.stack}`);
                    throw error;
                }
            }
        };
