    ...
    request.auth.sgSessionStorage = {
      ...
      authenticateWithHeaders: async function(headers, credentials = {}, options = {}) {
        try {
          ...
          const user = await server.plugins.searchguard
            .getSearchGuardBackend()
            .authenticateWithHeaders(headers, credentials, additionalAuthHeaders);
          const session = {
      ...
      getAuthInfo: async function() {
        ...
        try {
          const authInfo = await server.plugins.searchguard
            .getSearchGuardBackend()
            .authinfo(request.headers);
