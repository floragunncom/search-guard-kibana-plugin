...
export default function (pluginRoot, server, APP_ROOT, API_ROOT) {
    ...
    const unauthenticatedRoutes = config.get('searchguard.basicauth.unauthenticated_routes');
    // START add default unauthenticated routes
    unauthenticatedRoutes.push("/api/v1/systeminfo");
    ...
    server.auth.scheme('sg_access_control_scheme', (server, options) => ({
        authenticate: (request, reply) => {
            if (request.headers.authorization) {
                ...
                var credentials = server.plugins.searchguard.getSearchGuardBackend().getUser(username, password);
            ...
            if (unauthenticatedRoutes.includes(request.path)) {
                var credentials = server.plugins.searchguard.getSearchGuardBackend().getServerUser();
                reply.continue({credentials});
                return;
            };
