...
export default function (pluginRoot, server, rbacAppPermissions) {

    server.ext('onPostAuth', async function(request, reply) {
        ...
        try {
            // Toggle app links
            let appPermissionsResult = await rbacAppPermissions.getPermissionsResult(request);
            rbacAppPermissions.toggleNavLinkVisibility(appPermissionsResult.allowed);
