...
export const API_ACCESS_PREFIX = 'access:';
export const APP_ACCESS_PREFIX = 'kibana:app:';
...
export function toggleUiCapabilities(uiCapabilities, appPermissionsResult) {
    const result = mapValuesDeep(uiCapabilities, (value, key, obj, path) => {
        // If the value has been set to false elsewhere, we leave it as it is
        if (value === false) {
            return false;
        }
 ...
 */
export async function getPermissionsResult(searchguardBackend, request, appPermissions) {

    const result = await searchguardBackend.hasPermissions(request.headers, appPermissions.join(','));
