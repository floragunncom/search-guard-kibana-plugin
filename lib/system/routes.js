...
export default function(pluginRoot, server, APP_ROOT, API_ROOT) {
  server.route({
    ...
    handler: async (request, h) => {
      try {
        const data = await server.plugins.searchguard
          .getSearchGuardBackend()
          .systeminfo(request.headers);
  ...
  server.route({
    ...
    handler: async request => {
      try {
        return await server.plugins.searchguard
          .getSearchGuardBackend()
          .uploadLicense(request.headers, request.payload);
      } catch (error) {
