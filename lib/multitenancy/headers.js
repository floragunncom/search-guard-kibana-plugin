...
export default function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, authClass) {
    ...
    server.ext('onPreResponse', (request, h) => {
        try {
          assign(request.response.headers, {
            'sgtenant' : request.auth.sgSessionStorage.getStorage('tenant', {}).selected || ''
          });
        } catch (error) {
          // This is not necessarily an error, so we just log it as info
          server.log(['searchguard', 'info'], `Could not add the tenant to the response: ${error.message}. Path was ${request.path}`);
        }
    ...
    server.ext('onPreAuth', async function (request, h) {
        ...
        if (!request.path.startsWith("/internal/spaces")
