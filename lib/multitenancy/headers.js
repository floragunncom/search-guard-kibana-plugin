...
export default function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, authClass, config) {
    ...
    server.ext('onPreAuth', async function (request, h) {
        ...
        if (config.get('xpack.spaces.enabled') && (request.path === '/' || request.path.startsWith('/app'))) {
