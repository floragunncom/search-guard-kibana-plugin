...
export default function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, authClass) {
    ...
    server.ext('onPreAuth', async function (request, h) {
        ...
        if (!request.path.startsWith("/internal/spaces") && !request.path.startsWith("/elasticsearch") && !request.path.startsWith("/api") && !request.path.startsWith("/app") && request.path != "/" && selectedTenant === storedSelectedTenant) {
            return h.continue;
        }
