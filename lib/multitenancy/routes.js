...
module.exports = function(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, config) {
  ...
  server.route({
    ...
    handler: (request, h) => {
      ...
      const prefs = backend.updateAndGetTenantPreferences(request, username, selectedTenant);
  ...
  server.route({
    ...
    handler: request => {
      const mtinfo = server.plugins.searchguard
        .getSearchGuardBackend()
        .multitenancyinfo(request.headers);
      return mtinfo;
    },
  });
