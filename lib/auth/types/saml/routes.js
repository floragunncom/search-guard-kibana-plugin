...
module.exports = function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT) {
    ...
    const customErrorApp = kbnServer.apps.byId['searchguard-customerror'];
    ...
    server.route({
        ...
        handler: {
            async: async(request, reply) => {
                ...
                const redirectURL = (authInfo && authInfo.sso_logout_url) ? authInfo.sso_logout_url : `${APP_ROOT}${routesPath}login`;
