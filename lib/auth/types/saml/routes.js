...
export function loginHandler({ server, basePath = '' } = {}) {
  return async function(request, h) {
    ...
    try {
      ...
      server.log(['searchguard', 'error'], `An error occurred while obtaining the SAML header ${error}`);
      return h.redirect(basePath + '/customerror?type=samlConfigError');
    }
  };
}

export function acsHandler({ server, basePath = '', debugLog } = {}) {
  return async function(request, h) {
    ...
    try {
      ...
      debugLog({
        ...
        SAMLResponse: request.payload.SAMLResponse,
      });
...
export function samlLogoutHandler() {
...
export function authLogoutHandler({ config, server }) {
...
export function defineRoutes(server, debugLog) {
    ...
    server.route({
        ...
        handler: loginHandler({ server, basePath }),
    });
    ...
    server.route({
        ...
        handler: acsHandler({ server, basePath, debugLog }),
    });
    ...
    server.route({
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                debugLog('Got SAMLResponse: ' + request.payload.SAMLResponse);
   ...
   */
    ...
    server.route({
        ...
        handler: authLogoutHandler({ server, config }),
        options: {
            auth: false
        }
    });
