...
export default class Saml extends AuthType {
  // TODO: remove server (hapi) from arguments
  constructor(
    searchGuardBackend,
    server,
    APP_ROOT,
  ...
  onUnAuthenticated(request, response, toolkit, error) {
    ...
    if (error instanceof MissingTenantError) {
      redirectTo = `${this.basePath}/customerror?type=missingTenant`;
    } else if (error instanceof MissingRoleError) {
      redirectTo = `${this.basePath}/customerror?type=missingRole`;
    } else if (error instanceof AuthenticationError) {
      redirectTo = `${this.basePath}/customerror?type=samlAuthError`;
    } else {
      redirectTo = `${this.basePath}/auth/saml/login?nextUrl=${nextUrl}`;
    }
