...
export default class BasicAuth extends AuthType {
  ...
  handleUnauthenticated() {
    this.server.ext('onPreResponse', (request, h) => {
      try {
        ...
        this.server.log(
          ['searchguard', 'error'],
          `An error occurred while checking for 401 Unauthorized`
        );
      }
  ...
  async authenticate(credentials, options = {}, additionalAuthHeaders = {}) {
    ...
    try {
      const user = await this.server.plugins.searchguard
  ...
  onUnAuthenticated(request, h, error) {
    ...
    if (this.loadBalancerURL) {
      return h.redirect(
        `${this.loadBalancerURL}${this.basePath}${this.APP_ROOT}/login?nextUrl=${nextUrl}`
      );
    }
