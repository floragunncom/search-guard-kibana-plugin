...
export default class BasicAuth extends AuthType {
  ...
  onUnAuthenticated(request, response, toolkit, error = null) {
    ...
    let redirectTo = `${this.basePath}${APP_ROOT}/login?nextUrl=${nextUrl}`;
    ...
    if (this.anonymousAuthEnabled) {
      redirectTo = `${this.basePath}${APP_ROOT}/auth/anonymous?nextUrl=${nextUrl}`;
    }

    if (this.loadBalancerURL) {
      redirectTo = `${this.loadBalancerURL}${this.basePath}${APP_ROOT}/login?nextUrl=${nextUrl}`;
    }
