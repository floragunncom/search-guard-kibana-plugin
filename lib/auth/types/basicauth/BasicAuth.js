...
export default class BasicAuth extends AuthType {
  ...
  handleUnauthenticated() {
    this.server.ext('onPreResponse', (request, h) => {
      try {
        ...
        this.server.log(
          ['searchguard', 'error'],
          `An error occurred while checking for 401 Unauthorized: ${error.stack}`
        );
      }
  ...
  onUnAuthenticated(request, response, toolkit, error = null) {
    ...
    let redirectTo = `${this.basePath}${this.APP_ROOT}/login?nextUrl=${nextUrl}`;

    if (error && error instanceof MissingRoleError) {
      redirectTo = this.basePath + '/customerror?type=missingRole';
    }

    if (this.anonymousAuthEnabled) {
      redirectTo = `${this.basePath}${this.APP_ROOT}/auth/anonymous?nextUrl=${nextUrl}`;
    }

    if (this.loadBalancerURL) {
      redirectTo = `${this.loadBalancerURL}${this.basePath}${this.APP_ROOT}/login?nextUrl=${nextUrl}`;
    }
  ...
  setupRoutes() {
    require('./routes')(
      this, // @todo This is to handle the auth method. Maybe we should only inject "this"?
