            ...
            try {
                ...
                if (alternativeHeaders && alternativeHeaders.length) {
                    ...
                    if (foundHeaders.length) {
                        const hapiRequest = ensureRawRequest(request);
                        let {session} = await hapiRequest.auth.sgSessionStorage.authenticateWithHeaders(request.headers);
                ...
                server.log(['error', 'searchguard'], `An error occurred while checking for alternative login headers: ${error}`);
