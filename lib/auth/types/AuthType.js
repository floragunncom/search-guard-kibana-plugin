...
export default class AuthType {
    ...
    init() {
        throw new Error('The init method must be implemented by the sub class');
    }
    ...
    detectAuthHeaderCredentials(request, sessionCredentials) {
        throw new Error('The detectAuthHeaderCredentials method must be implemented by the sub class');
    }
        ...
        let validate = async(request, session, callback) => {
            ...
            if (differentAuthHeaderCredentials) {
                try {
                    ...
                    return callback(null, true, authResponse.session);
                } catch(error) {
    ...
    assignAuthHeader() {
        // @todo WHy async?
        this.server.ext('onPostAuth', async (request, next) => {
            if (request.auth.sgSessionStorage.isAuthenticated()) {
                const session = request.auth.sgSessionStorage.getSession();
                try {
                    let authHeader = this.getAuthHeader(session);
                    if (authHeader !== false) {
                        console.log(session.credentials.username)
