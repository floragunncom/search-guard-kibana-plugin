...
export default class AuthType {
    ...
    constructor(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT) {
        ...
        this.routesToIgnore = [{
            method: 'post',
            match: '/api/saved_objects/index-pattern'
        }];
    ...
    init() {
        this.deactivateAuthForRoutes(this.routesToIgnore);
        this.setupStorage();
    ...
    /**
     * Per default we check for auth on all routes, unless
     * a) the route is listed in "unauthenticatedRoutes" or
     * b) the auth option in the route definition is explicitly set to false
     *
     * We can define a given set of routes for which we just ignore auth altogether
     *
     * @param routesToIgnore - An array with objects defining the request "method" and "match" for the url pattern we are looking for
     */
    deactivateAuthForRoutes(routesToIgnore) {
        routesToIgnore.map((route) => {
            let routeDefinition = this.server.connections[0].match(route.method || 'get', route.match);
            if (routeDefinition && routeDefinition.settings) {
                routeDefinition.settings.auth = false;
            }
        });
    }
        ...
        this.server.auth.scheme('sg_access_control_scheme', (server, options) => ({
            authenticate: (request, reply) => {
                ...
                if (this.unauthenticatedRoutes.includes(request.path)) {
                    // @todo Why do we need to return the credentials here? I think they set the cookie
