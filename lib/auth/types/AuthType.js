...
export default class AuthType {
  ...
  checkAuth = async (request, response, toolkit) => {
    ...
    if (sessionCookie) {
      try {
        ...
        // @todo This is a weird construct - sessionValidationConstruct.credentials actually contains
      ...
      if (authHeaderCredentials) {
        try {
          ...
          this.logger.error(
            `Got auth header credentials, but authentication failed: ${authError.stack}`
          );
          return this.onUnAuthenticated(request, response, toolkit, authError);
        }
      }
    }
      ...
      if (request.headers) {
        ...
        if (
          ...
          return response.unauthorized({
            body: { message: 'Session expired', redirectTo: 'login' },
          });
        }
      }
      return this.onUnAuthenticated(request, response, toolkit);
  ...
  async assignAuthHeader(request) {
    ...
    // @todo This if clause should probably go - what about the additional auth headers?
    if (!request.headers[this.authHeaderName]) {
      ...
      if (sessionCookie) {
        try {
          const sessionValidationResult = await this.validateSessionCookie(request, sessionCookie);
          if (sessionValidationResult.valid) {
            // @todo This is a weird construct - sessionValidationConstruct.credentials actually contains
            // the entire cookie. This was due to the inner workings of hapi-auth-cookie
            sessionCookie = sessionValidationResult.credentials;
          } else {
            sessionCookie = false;
          }
        } catch (error) {
      ...
      if (sessionCookie && sessionCookie.credentials) {
        try {
          // @todo This is the part that needs to return the headers instead
          const authHeader = this.getAuthHeader(sessionCookie);
          if (authHeader !== false) {
            this.addAdditionalAuthHeaders(request, authHeader, sessionCookie);
   ...
   */
  clear(request) {
    return this.sessionStorageFactory.asScoped(request).clear();
  }
}
