...
export default class AuthType {
    ...
    async assignAuthHeader(request) {
        ...
        if (! request.headers[this.authHeaderName]) {
            ...
            if (session) {
                const sessionValidator = this.sessionValidator(this.server);
                try {
                    await sessionValidator(request, session, (error, valid, credentials) => {
                        if (valid) {
                            // The credentials we need are a bit nested
