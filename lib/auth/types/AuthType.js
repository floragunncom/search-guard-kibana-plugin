...
export default class AuthType {
    ...
    constructor(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT) {
        ...
        this.routesToIgnore = [
            ...
            '/bundles/app/searchguard-login/bootstrap.js',
    ...
    setupAuthScheme() {
        ...
        this.server.auth.scheme('sg_access_control_scheme', (server, options) => ({
            authenticate: async(request, h) => {
                ...
                if (this.unauthenticatedRoutes.includes(request.path)) {
                    credentials = this.server.plugins.searchguard.getSearchGuardBackend().getServerUser();
                ...
                try {
                    ...
                    if (authHeaderCredentials) {
                        try {
                            ...
                            this.debugLog('Got auth header credentials, but authentication failed');
        ...
        this.server.auth.strategy('sg_access_control', 'sg_access_control_scheme', this.getCookieConfig());
    ...
    debugLog(message, labels = null) {
      if (this.authDebugEnabled) {
        try {
          ...
          this.server.log(['searchguard', 'error'], `Error in debug log ${error}`);
        }
