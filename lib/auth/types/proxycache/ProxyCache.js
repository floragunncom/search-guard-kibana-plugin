...
export default class ProxyCache extends AuthType {
    ...
    constructor(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, kibanaCore = null, config = null) {

        super(pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, kibanaCore, config);
    ...
    onUnAuthenticated(request, h, error) {
        if (error instanceof MissingTenantError) {
            ...
            let loginEndpoint = this.config.get('searchguard.proxycache.login_endpoint');
            if (loginEndpoint) {
                try {
    ...
    setupRoutes() {
        require('./routes')(this.pluginRoot, this.server, this.kbnServer, this.APP_ROOT, this.API_ROOT, this.kibanaCore, this.config);
    }
