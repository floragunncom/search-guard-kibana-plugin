...
module.exports = function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, kibanaCore, kibanaConfig) {
    ...
    server.route({
        ...
        handler(request, h) {
            ...
            let loginEndpoint = config.get('searchguard.proxycache.login_endpoint');
            if (loginEndpoint) {
                try {
                    const redirectUrl = parseLoginEndpoint(loginEndpoint);
                    return h.redirect(redirectUrl);
                } catch(error) {
                    server.log(['error', 'searchguard'], 'An error occured while parsing the searchguard.proxycache.login_endpoint value');
                }
            } else {
