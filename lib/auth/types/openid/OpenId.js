...
export default class OpenId extends AuthType {
    ...
    async setupRoutes() {
        try {
            const searchGuardBackend = this.server.plugins.searchguard.getSearchGuardBackend();
          console.log('OIDC: Getting the endpoints')
            const oidcWellKnown = await searchGuardBackend.getOIDCWellKnown();

            console.log('OIDC: Got endpoints', {oidcWellKnown})
            ...
            require('./routes')(this.pluginRoot, this.server, this.kbnServer, this.APP_ROOT, this.API_ROOT, endPoints, this.debugLog.bind(this), searchGuardBackend);

            this.debugLog({endPoints})

        } catch (error) {
          console.log('OIDC: Something went wrong', error)
          throw new Error(`Failed when trying to obtain the endpoints from your IdP: ${error.stack}`);
        }
    }
}
