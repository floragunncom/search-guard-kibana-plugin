...
module.exports = function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, openIdEndPoints) {
    ...
    server.route({
        ...
        handler: (request, reply) => {
            ...
            const token = request.state[cookieName].credentials.token;
            let post_redirect_uri = `${baseRedirectUrl}${basePath}/app/kibana`;
            const endSessionUrl = openIdEndPoints.end_session_endpoint + '?post_logout_redirect_uri=' + post_redirect_uri + '&id_token_hint=' + token;
            
            // @todo Move tenant to sgSession?
            reply({redirectURL: endSessionUrl}).unstate('searchguard_tenant');
        },
        config: {
            auth: false
        }
