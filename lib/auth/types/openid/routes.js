...
module.exports = async function (pluginRoot, server, kbnServer, APP_ROOT, API_ROOT, openIdEndPoints, debugLog) {
            ...
            handler: async(request, h) => {
                ...
                try {
                    ...
                    let {user} = await request.auth.sgSessionStorage.authenticate({
                        ...
                        refreshToken: request.auth.artifacts['refresh_token'],
                    });

                    debugLog('OpenId response token: ' + request.auth.artifacts['id_token']);
    ...
    server.route({
        ...
        handler: (request, h) => {
            ...
            const token = request.auth.sgSessionStorage.getExtraStorage().split(' ')[1];
