 ...
 *
 */
export default function(backend, server, APP_ROOT, API_ROOT) {
  /**
  ...
  server.route({
    ...
    async handler(request) {
      try {
        ...
        if (request.auth && request.auth.sgSessionStorage) {
          request.auth.sgSessionStorage.clearStorage('authInfo');
        }
        return {
  ...
  server.route({
    ...
    async handler(request) {
      try {
        return await backend.restapiinfo(request.headers);
      } catch (error) {
        if (error.isBoom) {
          return enrichApiError(error);
        }
  ...
  server.route({
    ...
    async handler(request) {
      try {
        ...
        return await backend.indices({ headers, index });
      } catch (error) {
        if (error.isBoom) {
          return enrichApiError(error);
        }
  ...
  server.route({
    ...
    async handler(request) {
      try {
        ...
        return await backend.aliases({ headers, alias });
      } catch (error) {
        if (error.isBoom) {
          return enrichApiError(error);
        }
  ...
  server.route({
    ...
    async handler(request) {
      try {
        const response = await backend.validateDls(
          request.headers,
