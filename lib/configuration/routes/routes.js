 ...
 *
 */
export default function(pluginRoot, server, APP_ROOT, API_ROOT, config) {
  const backend = server.plugins.searchguard.getSearchGuardConfigurationBackend();
  ...
  server.route({
    ...
    async handler(request, h) {
      try {
        const response = backend.indices(request.headers);
        return response;
      } catch (error) {
        if (error.isBoom) {
          return enrichApiError(error);
