...
export default function (pluginRoot, server, APP_ROOT, API_ROOT) {
  ...
  server.route({
    ...
    handler: {
      async: async (request, reply) => {
        try {
          const instance = await backend.get(request.headers, request.params.resourceName, request.params.id);
  ...
  server.route({
    ...
    handler: (request, reply) => {
      try {
        let restapiinfo = backend.restapiinfo(request.headers);
        return reply(restapiinfo);
      } catch (error) {
        throw(error);
      }
    }
