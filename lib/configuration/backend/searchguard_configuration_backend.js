...
import Joi from 'joi';
import internalusersSchema from '../validation/internalusers';
import actiongroupsSchema from '../validation/actiongroups';
import rolesSchema from '../validation/roles';
import rolesmappingSchema from '../validation/rolesmapping';
import tenantsSchema from '../validation/tenants';
/**
 * The SearchGuard  backend.
 */
export default class SearchGuardConfigurationBackend {
  constructor(core, server, configService) {
    ...
    this.getValidator = (resourceName) => {
      switch (resourceName) {
        case 'internalusers':
          return internalusersSchema;
        case 'actiongroups':
          return actiongroupsSchema;
        case 'rolesmapping':
          return rolesmappingSchema;
        case 'roles':
          return rolesSchema;
        case 'tenants':
  ...
  async indices({ headers, index = [] } = {}) {
    try {
      const authHeaders = filterAuthHeaders(headers, this.requestHeadersWhitelist);
      const options = { format: 'json', h: 'index,health', index };

      return await this._cluster.callWithRequest({ headers: authHeaders }, 'cat.indices', options);
    } catch (error) {
      throw wrapElasticsearchError(error);
    }
  }
  ...
  async aliases({ headers, alias = [] } = {}) {
    try {
      const authHeaders = filterAuthHeaders(headers, this.requestHeadersWhitelist);
      const options = { format: 'json', h: 'index,alias', alias };

      return await this._cluster.callWithRequest({ headers: authHeaders }, 'cat.aliases', options);
    } catch (error) {
      throw wrapElasticsearchError(error);
    }
  }
