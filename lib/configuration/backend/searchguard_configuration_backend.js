...
import {roles_schema, rbac_roles_schema} from "../validation/roles";
...
export default class SearchGuardConfigurationBackend {
    ...
    constructor(server) {
        ...
        this.getValidator = (resourceName) => {
            switch (resourceName) {
                ...
                case 'roles':
                    const rbacSettings = server.plugins.searchguard.getSearchGuardSettings('rbac');
                    return (rbacSettings && rbacSettings.enabled) ? rbac_roles_schema : roles_schema;
                default:
    ...
    async list(headers, resourceName, query) {
        const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
        try {
