...
export default class SearchGuardBackend {
  constructor(core, server, serverConfig, esConfig) {
    ...
    this._cluster = core.elasticsearch.createClient('searchguard', {
  ...
  async authenticateWithHeader(headerName, headerValue, additionalAuthHeaders = {}) {
    try {
      ...
      const headers = filterAuthHeaders(
        additionalAuthHeaders,
        this._esconfig.requestHeadersWhitelist
      );
  ...
  updateAndGetTenantPreferences(request, user, tenant) {
    const preferencesCookieName = this.serverConfig.get(
  ...
  getTenantByPreference(
    ...
    if (prefs) {
      const preferredTenant = prefs[username];
      ...
      if (
        (preferredTenant == '__user__' || preferredTenant == 'private') &&
