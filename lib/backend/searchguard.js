...
export default class SearchGuardBackend {
    ...
    async authenticate(credentials) {
        ...
        try {
            ...
            let response = await this._client('searchguard.authinfo', {
                headers: {
    ...
    buildSessionResponse(credentials, authInfoResponse) {
        return new User(authInfoResponse.user_name, credentials, null, authInfoResponse.sg_roles, authInfoResponse.backend_roles, authInfoResponse.sg_tenants, authInfoResponse.user_requested_tenant);
    }
    ...
    async getOIDCWellKnown() {
      try {
        return await this._client('searchguard.oidcwellknown', {});
      } catch (error) {
        throw wrapElasticsearchError(error);
      }
    }
   ...
   */
    async getOIDCToken({ tokenEndpoint, body }) {
      if (tokenEndpoint.indexOf('/_searchguard/') === 0) {
        // The es client plugin has '_/searchguard/' hardcoded,
        // so we need to remove it from the URL. Having the
        // entire endpoint as a dynamic URL in the plugin
        // would cause the URL to be encoded, which led to
        // a MethodNotAllowed for POST requests
        tokenEndpoint = tokenEndpoint.replace('/_searchguard/', '');
      }
      return await this._client('searchguard.oidcauthtoken', {
        tokenEndpoint,
        body,
      });
    }
    ...
    async getKibanaInfoWithInternalUser() {
        try {
            return await this._cluster.callWithInternalUser('searchguard.multitenancyinfo', {});
        } catch(error) {
            throw wrapElasticsearchError(error);
        }
    }
    ...
    async hasPermissions(headers, permissions) {
        try {
            const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
            return await this._client('searchguard.permissions', {
                headers: authHeaders,
                permissions,
            });
        } catch(error) {
    ...
    async getTenantInfoWithInternalUser() {
        try {
            return await this._cluster.callWithInternalUser('searchguard.tenantinfo', {});
        } catch(error) {
            throw wrapElasticsearchError(error);
        }
    }
    ...
    async getTenantInfo(headers) {
        try {
            const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
            const response = await this._client('searchguard.tenantinfo', {
                headers: authHeaders
            });
            return response
        } catch(error) {
    ...
    updateAndGetTenantPreferences(request, user, tenant) {
        const preferencesCookieName = this.server.config().get('searchguard.cookie.preferences_cookie_name');

        var prefs = request.state[preferencesCookieName];
