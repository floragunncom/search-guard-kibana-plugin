...
export default class SearchGuardBackend {
    ...
    async getKibanaInfo(headers) {
        try {
            const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
            const response = await this._client.searchguard.kibanaInfo({
                headers: authHeaders
            });
            return response
        } catch(error) {
    ...
    // TODO RBAC: I think this is not needed for 7.x
    // async upgradeRolesFormat(headers) {
    //     const body = {
    //         action: 'formatUpgrade'
    //     };
    //
    //     try {
    //         const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
    //         const response = await this._client.searchguard.upgradeRolesFormat({
    //             headers: authHeaders,
    //             body
    //         });
    //         return response
    ...
    async hasPermissions(headers, permissions) {
        try {
            const authHeaders = filterAuthHeaders(headers, this._esconfig.requestHeadersWhitelist);
            const response = await this._client.searchguard.permissions({
                headers: authHeaders,
                permissions,
            })
