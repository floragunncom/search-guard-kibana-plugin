...
export default class BasicAuth extends AuthType {
  constructor(searchGuardBackend, server, APP_ROOT, API_ROOT, kibanaCore, config, logger) {
    super(searchGuardBackend, server, APP_ROOT, API_ROOT, kibanaCore, config, logger);
  ...
  handleUnauthenticated() {
    this.server.ext('onPreResponse', (request, h) => {
      try {
        ...
        this.server.log(['searchguard', 'error'], `An error occurred while checking for 401 Unauthorized: ${error.stack}`);
      }
