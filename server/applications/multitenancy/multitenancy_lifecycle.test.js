...
function setupGetStartServices({ elasticsearchClientAsScoped = jest.fn() } = {}) {
  return () => {
...
function setupForCreateDefaultSpace() {
  ...
  const getStartServices = setupGetStartServices({ elasticsearchClientAsScoped });
...
describe('MultitenancyLifecycle.onPreAuth', () => {
  ...
  test('tenant is read from the cookie and validated', async () => {
    ...
    const { getStartServices } = setupForCreateDefaultSpace();
