...
export function getSecurityCookieOptions(configService) {
  const options = {
    encryptionKey: configService.get('eliatra.security.cookie.password'),
    name: configService.get('eliatra.security.cookie.name'),
    isSecure: configService.get('eliatra.security.cookie.secure'),
    ...
    clearInvalid: true,
    ttl: configService.get('eliatra.security.cookie.ttl'),
    sameSite: configService.get('eliatra.security.cookie.isSameSite'),
  };

  if (configService.get('eliatra.security.cookie.domain')) {
    options.domain = configService.get('eliatra.security.cookie.domain');
  }
