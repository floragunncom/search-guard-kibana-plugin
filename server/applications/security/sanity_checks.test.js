...
import { setupLoggerMock, setupeliatraSuiteBackendMock } from '../../utils/mocks';
...
describe('sanity_checks', () => {
  ...
  describe('checkDoNotFailOnForbidden', () => {
    let mockLogger;
    let mockeliatraSuiteBackend;

    beforeEach(() => {
      mockLogger = setupLoggerMock();
      mockeliatraSuiteBackend = setupeliatraSuiteBackendMock();
    });

    test('log warning if not not_fail_on_forbidden_enabled', async () => {
      mockeliatraSuiteBackend.getKibanaInfoWithInternalUser = jest.fn().mockResolvedValue({
        not_fail_on_forbidden_enabled: false,
      });

      await checkDoNotFailOnForbidden({
        eliatraSuiteBackend: mockeliatraSuiteBackend,
        logger: mockLogger,
      });
    ...
    test('handle rejection', async () => {
      ...
      mockeliatraSuiteBackend.getKibanaInfoWithInternalUser = jest.fn().mockRejectedValue(error);
    ...
    test('log a warning if the cookie password is the default password', () => {
      const configService = new ConfigService({
        eliatra: {
          security: {
            cookie: {
              password: DEFAULT_CONFIG.eliatra.security.cookie.password,
            },
          }
        },
      });
