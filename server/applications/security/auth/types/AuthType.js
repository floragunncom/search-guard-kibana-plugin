...
export default class AuthType {
  constructor({
    ...
    this.frontendBaseUrl =
      this.config.get('eliatra.security.frontend_base_url') || kibanaCore.http.basePath.publicBaseUrl;
    this.sgFrontendConfigId = this.config.get('eliatra.security.sp_frontend_config_id') || 'default';
    ...
    this.authDebugEnabled = this.config.get('eliatra.security.auth.debug');
     ...
     * @type {string[]}
     */
    this.allowedAdditionalAuthHeaders = ['sp_impersonate_as'];
  }
  ...
  /**
   * Returns the auth header needed for the Eliatra Suite Security backend
   * @param session
   * @returns {*}
   */
  ...
  async authenticate(credentials, options = {}, additionalAuthHeaders = {}) {
    try {
      ...
      const sessionResponse = await this.eliatraSuiteBackend.authenticateWithSession(credentials);
      ...
      const user = await this.eliatraSuiteBackend.authenticateWithHeader(
        this.authHeaderName,
    ...
    if (request.headers) {
      ...
      if (
        ...
        return response.unauthorized({
          headers: {
            sp_redirectTo: await this.getRedirectTargetForUnauthenticated(
              request,
              error,
              true,
              sessionCookie
  ...
  async validateSessionCookie(request, sessionCookie) {
    ...
    if (
      ...
      try {
        ...
        const authInfoResponse = await this.eliatraSuiteBackend.authinfo(
  ...
  async _handleAuthResponse(request, credentials, authResponse, additionalAuthHeaders = {}) {
    ...
    if (
      ...
      this.config.get('eliatra.security.multitenancy.enabled') &&
      !this.config.get('eliatra.security.multitenancy.tenants.enable_global')
    ) {
      ...
      if (allTenants != null) {
        ...
        // so we need to filter out the GLOBAL_TENANT too
        delete allTenants.GLOBAL_TENANT;
      }
    ...
    /* @todo Was used with sp_impersonate_as
  ...
  async clear(request, explicitlyRemoveAuthType = false) {
    ...
    if (authHeaders) {
      try {
        await this.eliatraSuiteBackend.logoutSession(authHeaders);
      } catch (error) {
