...
export class Kerberos {
  constructor(props) {
    ...
    this.eliatraSuiteBackend = props.eliatraSuiteBackend;
  ...
  async authenticateWithSPNEGO(request, response, toolkit, sessionCookie) {
    ...
    try {
      const whitelistRoutes = this.config.get('eliatra.security.auth.unauthenticated_routes');
      ...
      const createSessionResponse = await this.eliatraSuiteBackend.createSessionWithHeaders(headers);
  ...
  checkAuth = async (request, response, toolkit) => {
	...
	if (sessionCookie.credentials && sessionCookie.credentials.authHeaderValue) {
       ...
       try {
          const authInfoResponse = await this.eliatraSuiteBackend.authinfo(headers);
       } catch (e) {
