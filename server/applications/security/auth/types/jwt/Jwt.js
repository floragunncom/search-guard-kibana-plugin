...
export default class Jwt extends AuthType {
  ...
  async detectCredentialsByRequest({ request }) {
    if (!this.config.get('eliatra.security.auth.jwt_param.enabled')) {
      return null;
    }

    const urlparamname = this.config.get('eliatra.security.auth.jwt_param.url_param').toLowerCase();
  ...
  async getRedirectTargetForUnauthenticated(request, error = null, isAJAX = false) {
    ...
    if (error instanceof MissingTenantError) {
      ...
      const authConfig = (
        await this.eliatraSuiteBackend.getAuthConfig()
      ...
      if (loginEndpoint) {
        try {
          ...
          if (!isAJAX && !url.searchParams.has('nextUrl')) {
            ...
            // Delete sp_tenant because we have it already as a param in the nextUrl
            url.searchParams.delete('sp_tenant');
          }

          return url.toString();
        } catch (error) {
          this.logger.error(
            'An error occured while parsing the eliatrasuite.jwt.login_endpoint value'
          );
        }
      }
    }
