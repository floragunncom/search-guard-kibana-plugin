...
export function loginAuthHandler({ config, authInstance, logger, eliatraSuiteBackend }) {
  return async function (context, request, response) {
    ...
    try {
      ...
      const basicAuthConfig = config.get('eliatra.security.basicauth');
      ...
      if (config.get('eliatra.security.multitenancy.enabled')) {
        return response.ok({
          body: {
...
export function defineRoutes({
  ...
  router.post(
    ...
    loginAuthHandler({ config, authInstance, logger, eliatraSuiteBackend })
  );
} //end module
