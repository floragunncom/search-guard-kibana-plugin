...
export function defineRoutes({
  ...
  router.get(
    ...
    async function (context, request, response) {
      ...
      When successful logout,
        headers: {
          cookie: 'eliatrasuite_authentication='
        },
      */
      ...
      try {
        ...
        const authConfig = (
          await eliatraSuiteBackend.getAuthConfig(nextUrl)
        ...
        if (error.meta?.body?.error) {
	     cookies.push('sp_err=' + encodeURIComponent(error.meta?.body?.error) + "; Path=/");
	    } else if (error.message) {
	     cookies.push('sp_err=' + encodeURIComponent(error.message) + "; Path=/");
	    }

        if (error.meta?.body?.debug) {
	     cookies.push('sp_dbg=' + encodeURIComponent(JSON.stringify(error.meta?.body?.debug)) + "; Path=/");
	    }
  ...
  */
  router.post(
    {
      path: `${APP_ROOT}/security/saml/acs`,
      options: {
        authRequired: false,
        xsrfRequired: false,
      },
  ...
  router.post(
    {
      path: `${APP_ROOT}/security/saml/acs/idpinitiated`,
      options: {
        authRequired: false,
        xsrfRequired: false,
      },
   ...
   * back to Kibana.
   */
  const logoutPath = `${APP_ROOT}/security/saml/logout`;
  const logoutOptions = {
    authRequired: false,
    xsrfRequired: false,
  };
