...
export function defineAuthRoutes({ kibanaCore, authManager, eliatraSuiteBackend, configService }) {
  ...
  router.get(
    ...
    authConfigHandler({ authManager, eliatraSuiteBackend, configService, kibanaCore })
  );
...
export function authConfigHandler({ authManager, eliatraSuiteBackend, configService, kibanaCore }) {
  return async function (context, request, response) {
    const authConfig = await eliatraSuiteBackend.getAuthConfig();
