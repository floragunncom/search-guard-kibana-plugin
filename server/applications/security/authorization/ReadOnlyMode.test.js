  ...
  describe('register capabilities handler', () => {
    ...
    describe('read only by role', () => {
      test('detects read only role', async () => {
        const logger = setupLoggerMock();
        const authInfo = {
          effective_roles: ['KIBANA_USER', 'OWN_INDEX', 'readonly_role'],
          user_name: 'bruce',
           effective_tenants: {
            bruce: true,
            readwrite_1: true,
      ...
      test('ignores readonly role when no matching role found', async () => {
        const logger = setupLoggerMock();
        const authInfo = {
          effective_roles: ['KIBANA_USER', 'OWN_INDEX'],
        };
        const configRoles = ['readonly_role'];
        const readOnly = new ReadOnlyMode(logger);
      ...
      test('hides Multitenancy correctly for readonly role with only one tenant', () => {
        ...
        const authInfo = {
          ...
          user_name: 'bruce',
           effective_tenants: {
            ...
            readwrite_1: true,
            GLOBAL_TENANT: true, // Global disabled in config
          },
        };
        ...
        const configServiceMock = {
          get: jest.fn((path) => {
            ...
            if (path === 'eliatra.security.multitenancy.tenants.enable_private') return false;
            if (path === 'eliatra.security.readonly_mode.roles') return ['readonly_role'];
          }),
        };
        ...
        const originalCapabilities = {
          navLinks: {
            'eliatrasuite-multitenancy': true,
          },
        };
        ...
        expect(toggledCapabilities).toEqual({
          navLinks: {
            'eliatrasuite-multitenancy': false,
          },
          dashboard: {
            createNew: false,
      ...
      test('sets capabilities correctly for readonly role with more than one tenant', () => {
        ...
        const authInfo = {
          ...
          user_name: 'bruce',
           effective_tenants: {
            bruce: true, // Disabled in config
            readwrite_1: true,
            GLOBAL_TENANT: true, // Global ENABLED in config
          },
        };
        ...
        const configServiceMock = {
          get: jest.fn((path) => {
            if (path === 'eliatra.security.multitenancy.enabled') return true;
            if (path === 'eliatra.security.multitenancy.tenants.enable_global') return true;
            if (path === 'eliatra.security.multitenancy.tenants.enable_private') return false;
            if (path === 'eliatra.security.readonly_mode.roles') return ['readonly_role'];
          }),
        };
    ...
    test('register read-only mode if read-only role', async () => {
      ...
      const eliatraSuiteBackend = setupeliatraSuiteBackendMock({ authinfo });
