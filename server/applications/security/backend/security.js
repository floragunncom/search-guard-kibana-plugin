...
import { AUTHINFO_ENDPOINT} from '../../../utils/constants';
import { DASHBOARDSINFO_ENDPOINT } from '../../../utils/constants';
import { TENANTINFO_ENDPOINT } from '../../../utils/constants';
import { PERMISSIONSSINFO_ENDPOINT } from '../../../utils/constants';
import { LICENSEINFO_ENDPOINT } from '../../../utils/constants';
import { BACKEND_AUTH_BASEURL } from '../../../utils/constants';
import { BACKEND_RESTAPI_BASEURL } from '../../../utils/constants';

/**
 * The Eliatra Suite backend.
 */
export default class EliatraSuiteBackend {
  ...
  getAuthConfig = async (nextUrl = null) => {
    try {
      const sgFrontendConfigId = this.configService.get('eliatra.security.sp_frontend_config_id') || 'default';
	  let frontendBaseUrl = this.configService.get('eliatra.security.frontend_base_url') || this.core.http.basePath.publicBaseUrl;
      ...
      const response = await this._client({
        path: `${BACKEND_AUTH_BASEURL}/config`,
        method: 'POST',
  ...
  async authenticateWithSession(credentials) {
    try {
      const response = await this._client({
        path: `${BACKEND_AUTH_BASEURL}/session`,
        method: 'POST',
        body: credentials,
      });
  ...
  authenticate = async (credentials) => {
    ...
    try {
      ...
      return new User(
        ...
        credentials,
        credentials,
        response.effective_roles,
        response.backend_roles,
        response. effective_tenants,
  ...
  createSessionWithHeaders = async (headers, additionalAuthHeaders = {}) => {
    ...
    try {
	  return await this._client({
        path: `${BACKEND_AUTH_BASEURL}/session/with_header`,
        method: 'POST',
        headers,
      });
    } catch (error) {
  ...
  buildSessionResponse(credentials, authInfoResponse) {
    return new User(
      ...
      credentials,
      null,
      authInfoResponse.effective_roles,
  ...
  // TODO: This seems to be referenced from tests only, check if still needed
  getKibanaInfoWithInternalUser = async () => {
    try {
      return await this._client({
        path: DASHBOARDSINFO_ENDPOINT,
        method: 'get',
        asWho: 'asInternalUser',
      });
    } catch (error) {
  ...
  hasPermissions = async (headers, permissions) => {
    try {
      return await this._client({
        path: PERMISSIONSSINFO_ENDPOINT,
        method: 'get',
        headers,
  ...
  uploadLicense = async (headers, body) => {
    try {
      return await this._client({
        path: `${BACKEND_RESTAPI_BASEURL}/license`,
        method: 'put',
        headers,
        body,
      });
  ...
  updateAndGetTenantPreferences(request, user, tenant) {
    ...
    const preferencesCookieName = this.configService.get(
      'eliatra.security.cookie.preferences_cookie_name'
    );
