 ...
 */
uiModules.get('apps/searchguardConfiguration', []).service('backendRoles', function (backendAPI) {
  ...
  this.save = (rolename, data) => {
    ...
    const resourceCopy = JSON.parse(JSON.stringify(data));
    const newData = this.preSave(resourceCopy);
    return backendAPI.save(RESOURCE, rolename, newData);
  };
  ...
  this.preSave = (role) => {
    ...
    const clusterpermissions = backendAPI.mergeCleanArray(role.cluster.actiongroups, role.cluster.permissions);
    ...
    for (const indexname in role.indices) {
      if (role.indices.hasOwnProperty(indexname)) {
        const index = role.indices[indexname];

        for (const doctypename in index) {
          if (index.hasOwnProperty(doctypename)) {
            const doctype = index[doctypename];
            const doctypepermissions = backendAPI.mergeCleanArray(doctype.actiongroups, doctype.permissions);
        ...
        const dlsfls = role.dlsfls[indexname];
        if(dlsfls) {
          if (dlsfls._dls_.length > 0) {
            // remove any formatting
            let dls = dlsfls._dls_;
            try {
              const dlsJsonObject = JSON.parse(dls);
    ...
    // tenants
    role.tenants = {};
    for (let i = 0, l = role.tenantsArray.length; i < l; i++) {
      const tenant = role.tenantsArray[i];
  ...
  this.postFetch = (role) => {
    ...
    const clusterpermissions = backendAPI.sortPermissions(role.cluster);
    role.cluster = {};
    ...
    if (role.indices) {
      ...
      for (const indexname in role.indices) {
        if (role.indices.hasOwnProperty(indexname)) {
          ...
          for (const doctypename in index) {
            if (index.hasOwnProperty(doctypename)) {
              let doctype = index[doctypename];
              const doctypepermissions = backendAPI.sortPermissions(doctype);
              doctype = {
    ...
    role.tenantsArray = [];
    if (role.tenants) {
      const tenantNames = Object.keys(role.tenants).sort();
  ...
  this.determineFlsMode = function (dlsfls) {
    ...
    // any fields to set?
    const flsFields = dlsfls._fls_;
    if (isEmpty(flsFields) || !Array.isArray(flsFields)) {
      return;
    }
    for (let index = 0; index < flsFields.length; ++index) {
      const field = flsFields[index];
      if (field.startsWith('~')) {
  ...
  this.isRoleEmpty = function (role) {
    ...
    const clusterPermsEmpty = role.cluster.actiongroups.length === 0 && role.cluster.permissions.length === 0;
    const indicesEmpty = this.checkIndicesStatus(role).allEmpty;
  ...
  this.checkIndicesStatus = function (role) {
    ...
    if (role.indices) {
      ...
      const indexNames = Object.keys(role.indices);
      indexNames.forEach(function (indexName) {
        const docTypeNames = Object.keys(role.indices[indexName]);
        docTypeNames.forEach(function (docTypeName) {
          const doctype = role.indices[indexName][docTypeName];
