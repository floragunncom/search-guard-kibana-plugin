...
export const isSinglePermission = (permission = '') => {
  return permission.startsWith('cluster:')
    ...
    || permission.startsWith('alerting:');
};
...
export function buildEliatraSecurityConfiguration({ restapiinfo, eliatra }) {

  const eliatrasuiteConfiguration = cloneDeep(eliatra.security.configuration);
  const pagesConfiguration = Object.keys(PAGE_NAMES).reduce((pagesConfiguration, pageName) => {
    pagesConfiguration[pageName] = {
      enabled:
        eliatrasuiteConfiguration[pageName].enabled &&
  ...
  assign(eliatrasuiteConfiguration, pagesConfiguration);
  return eliatrasuiteConfiguration;
}
