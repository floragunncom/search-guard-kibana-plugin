  ...
  return (
    <EuiOverlayMask onClick={closeModal}>
      <EuiModal onClose={closeModal} style={{ maxWidth: 800 }} maxWidth={false}>
        <EuiModalHeader data-test-subj={`sp.userMenu.appListModal.header.${appModal.id}`}>
          <EuiModalHeaderTitle>
            <h1>{title}</h1>
          </EuiModalHeaderTitle>
        </EuiModalHeader>

        <EuiModalBody data-test-subj={`sp.userMenu.appListModal.body.${appModal.id}`}>
          {body}
          <EuiSpacer />
        </EuiModalBody>

        <EuiModalFooter data-test-subj={`sp.userMenu.appListModal.footer.${appModal.id}`}>
          <EuiButton onClick={closeModal} fill data-test-subj="sp.closeModal">
            {closeText}
          </EuiButton>
        </EuiModalFooter>
      </EuiModal>
...
export function HeaderUserMenu({ httpClient, configService }) {
  ...
  const authType = configService.get('eliatra.security.auth.type');
  const isAccountInfoEnabled = configService.get('eliatra.security.accountinfo.enabled');
  ...
  const popoverButton = (
    <EuiButtonEmpty onClick={openPopover} data-test-subj="sp.userMenu.button">
  ...
  if (authType === 'default') {
    ...
    appList.push({
      ...
      color: 'primary',
      onClick: logOut,
      'data-test-subj': 'sp.userMenu.button-logout',
    });
  }
      ...
      <EuiToolTip position="bottom" content={userNameTooltipText}>
        <EuiText>
          {isAccountInfoEnabled ? (
            <EuiButtonEmpty
              id="sp.userMenu.username"
              data-test-subj="sp.userMenu.username"
              onClick={openAccountModal}
            >
              {userName}
            </EuiButtonEmpty>
          ) : (
            <p style={{ wordWrap: 'break-word' }} id="sp.userMenu.username">
              {userName}
            </p>
          )}
        </EuiText>
