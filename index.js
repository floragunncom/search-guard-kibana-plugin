...
const { migrateTenants } = require('./lib/multitenancy/migrate_tenants');
    ...
    async init(server) {
      ...
      if (authType && authType !== '' && ['basicauth', 'jwt', 'openid', 'saml', 'proxycache'].indexOf(authType) > -1) {
        try {
          ...
          if (config.get('searchguard.cookie.password') === 'searchguard_cookie_default_password') {
            this.status.yellow(
              'Default cookie password detected, please set a password in kibana.yml by ' +
              'setting \'searchguard.cookie.password\' (min. 32 characters).'
            );
          }

          if (!config.get('searchguard.cookie.secure')) {
            this.status.yellow(
              '\'searchguard.cookie.secure\' is set to false, cookies are transmitted over' +
              ' unsecure HTTP connection. Consider using HTTPS and set this key to \'true\''
            );
          }
      ...
      if (config.get('searchguard.multitenancy.enabled')) {
        ...
        if (headersWhitelist.indexOf('sgtenant') === -1) {
          this.status.red(
            'No tenant header found in whitelist. Please add sgtenant to ' +
            'elasticsearch.requestHeadersWhitelist in kibana.yml'
          );
          return;
        }
      ...
      if ((typeof config.get('elasticsearch.ssl.certificate') !== 'undefined' &&
        config.get('elasticsearch.ssl.certificate') !== false) &&
        config.get('searchguard.allow_client_certificates') !== true) {
        this.status.red(
          '\'elasticsearch.ssl.certificate\' can not be used without setting ' +
          '\'searchguard.allow_client_certificates\' to \'true\' in kibana.yml. ' +
          'Please refer to the documentation for more information about the implications of doing so.'
        );
      }
    }
  });
